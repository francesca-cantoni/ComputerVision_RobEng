...
% using as a template a cutted image that keeps into account many information
...
function accuracy = big_template(IN_gray_scale, car_gray_big, i, fontSize)
    
    figure, sgtitle(['Detect patch inside original image # ',num2str(i), ' BIG'],'FontSize', fontSize);
    ...
    NCC = normxcorr2(car_gray_big, IN_gray_scale(:,:,:,1));
    
    [row, col] = size(car_gray_big);
    ...
    subplot(2,2,2), imagesc(car_gray_big), title('Big window'), colormap gray, set(gca, 'xtick',[],'ytick',[]),truesize([200 200]);
    ...
    drawnow;
    var_1 = int2str(i);
    var_1 = ['Detect_patch_inside_original_image_BIG_', var_1];
    print( var_1, '-dpng');
    ...
    X = ['BIG WINDOW -->  ','   x:', num2str(xpeak) , '   y:', num2str(ypeak) ];
    disp(X)
    ...
    figure, sgtitle(['Accuracy # ',num2str(i), ' BIG'],'FontSize', fontSize);
    patch = IN_gray_scale(yoffSet+1:yoffSet+row, xoffSet+1:xoffSet+col, :, 1);
    subplot(2,2,1), imagesc(IN_gray_scale(:,:,:,1)), title('Original image gray'), colormap gray, set(gca, 'xtick',[],'ytick',[]);
    subplot(2,2,2), imagesc(car_gray_big), title('Big window'), colormap gray, truesize([200 200]);
    subplot(2,2,3), imagesc(patch), title('Detected patch inside original image'), colormap gray;
    disp(patch);
    accuracy = double(patch) - double(car_gray_big);
    A = ['ACCURACY using BIG template and relative to image # ', num2str(i)];
    disp(A);
    disp(accuracy);
    ...
    drawnow;
    var_2 = int2str(i);
    var_2 = ['Accuracy_image_BIG_', var_2];
    print( var_2 , '-dpng');
end
